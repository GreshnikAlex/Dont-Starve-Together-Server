...
--	self.HistoryButton:SetText("Вернуть лог")
    ...
    self.updateoptionspanelbg = self.updateoptionspanel:AddChild(Image("images/fepanels.xml", "panel_topmods.tex"))
	self.updateoptionspanelbg:SetPosition(0,-65)
	self.updateoptionspanelbg:SetScale(1.55,0.95, 1)
	...
	self.TranslationTypeText = self.updateoptionspanel:AddChild(Text(UIFONT, 34))
	self.TranslationTypeText:SetHAlign(ANCHOR_LEFT)
	self.TranslationTypeText:SetRegionSize( 320, 70 )
	self.TranslationTypeText:SetPosition(7, -150, 0)
	...
	self.TranslationTypeOptions={{text = "Полная", data = t.TranslationTypes.Full, description = "Переведено всё, включая интерфейс, чат, названия предметов и реплики персонажей."},
					...
					{text = "Только чат", data = t.TranslationTypes.ChatOnly, description = "Подойдёт тем, кто не хочет перевода игры. В чате можно будет писать по русски."}}



	self.TranslationTypeSpinner = self.updateoptionspanel:AddChild(Spinner( self.TranslationTypeOptions, 350 ))
	...
	self.TranslationTypeSpinner:SetPosition(0, -200, 0)
...
--[[	self.updateoptionsspinner:SetFocusChangeDir(MOVE_DOWN, self.URLButton)
	...
	self.updateoptionsspinner:SetFocusChangeDir(MOVE_DOWN, self.TranslationTypeSpinner)
